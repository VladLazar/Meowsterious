...
void display_char(char c, uint16_t col)
{
    uint16_t x, y;
    const uint8_t* fdata;
    uint8_t bits, mask;
    uint16_t sc=display.x, ec=display.x + 15, sp=display.y, ep=display.y + 7;
...
void display_char_xy(char c, uint16_t x, uint16_t y, uint16_t col)
{
...
void display_string(char *str, uint16_t col)
{
    uint8_t i;
    for(i=0; str[i]; i++) 
        display_char(str[i], col);
}

void display_string_xy(const char *str, uint16_t x, uint16_t y, uint16_t col)
{
    uint8_t i;
    display.x = x;
    display.y = y;
    for(i=0; str[i]; i++)
        display_char_xy(str[i], x + i * (FONT_WIDTH - TEXT_OVERLAP), y, col);
}
